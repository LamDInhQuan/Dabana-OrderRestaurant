 ...
 *
 * Business rule: moi reservation chi co dung 1 lenh thu coc DANG HIEU LUC
 * (status PENDING/PROCESSING) tai 1 thoi diem - enforce o service, KHONG phai
 * UNIQUE DB, vi can cho phep tao lai link moi khi link cu CANCELLED/EXPIRED.
 */
@Getter
@Setter
@Entity
...
public class DepositPayment extends BaseEntity {
    ...
    // KHONG unique=true: 1 reservation co the co nhieu ban ghi DepositPayment
    // theo thoi gian (lan truoc CANCELLED/EXPIRED thi tao lai link moi). Rang
    // buoc "chi 1 lenh thu DANG HIEU LUC" duoc kiem tra o service bang
    // DepositPaymentRepository#existsByReservation_IdAndStatus(reservationId, PENDING/PROCESSING),
    // khong phai bang DB constraint.
