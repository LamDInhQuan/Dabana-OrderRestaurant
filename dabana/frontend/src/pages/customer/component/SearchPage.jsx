...
export default function SearchPage() {
    ...
    useEffect(() => {
        ...
        const params = { keyword, province, cuisine, minRate }
        ...
        restaurantApi.search(params)
            .then(res => {
                ...
                const filtered = extractedBranches.filter(branch => {
                    ...
                    if (minRate && minRate.trim() !== '') {
                        const targetRate = parseFloat(minRate)
                        const branchRate = branch.rate || 0
                        if (branchRate < targetRate) return false
                    }
                    return true
                })
                                        ...
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                                            ...
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '4px', background: '#fffbeb', border: '1px solid #fef3c7', padding: '2px 8px', borderRadius: '12px' }}>
                                                ...
                                                <span style={{ fontSize: '0.78rem', fontWeight: 700, color: '#b45309' }}>
                                                    {branch.rate ? branch.rate.toFixed(1) : '0.0'}
                                                </span>
                                            </div>
                                        </div>
