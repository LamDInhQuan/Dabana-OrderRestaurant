...
public class SubscriptionBillingScheduler {
        ...
        /**
         * Han muc chi nhanh toi thieu ap dung tam thoi khi subscription EXPIRED, dung
         * de
        ...
        public void generateRenewalInvoices() {
                ...
                for (RestaurantSubscription subscription : dueForRenewal) {
                        ...
                        // Neu co lich ha cap dang cho, dung goi do de tinh hoa don gia han; nguoc lai
                        // dung goi hien tai.
                        SubscriptionPlan planToRenew = subscription.getPendingDowngradePlan() != null
                                        ? subscription.getPendingDowngradePlan()
                                        : subscription.getPlan();

                        LocalDate periodEnd = BillingCycleUtils.addOneCycle(expectedRenewalPeriodStart,
                                        planToRenew.getBillingCycle());
                        ...
                        // Tu dong tao link thanh toan ngay (khac voi INITIAL/UPGRADE - nha hang chu
                        // dong bam
                        // dang ky/nang cap nen tao link ngay luc do; con RENEWAL phat sinh tu cron nen
                        // can co
                        // san link truoc de gui kem trong thong bao, khong bat nha hang phai vao app
                        // roi moi tao).
                        try {
                                subscriptionService.createPaymentLinkForInvoice(subscription.getRestaurant().getId(),
                                                invoice.getId());
                        } catch (Exception e) {
        ...
        public void markOverdueAndStartGracePeriod() {
                ...
                for (SubscriptionInvoice invoice : overdueInvoices) {
                        if (invoice.getInvoiceType() == InvoiceType.UPGRADE) {
                                // Hoa don UPGRADE qua han khong anh huong subscription dang ACTIVE (van dung
                                // goi cu) -
                        ...
                        if (subscription.getStatus() == SubscriptionStatus.ACTIVE
                                ...
                                notificationService.sendImmediate(
                                                ...
                                                "Hoá đơn gói " + invoice.getPlanSnapshotName()
        ...
        public void expireAndSuspendOverLimitBranches() {
                ...
                for (RestaurantSubscription subscription : pastDueExpired) {
                        ...
                        notificationService.sendImmediate(
                                        ...
                                        "Gói dịch vụ hết hạn đã qua thời gian ân hạn. Đã tạm ngưng "
                                                        + suspendedBranchIds.size()
                                                        + " chi nhánh. Thanh toán lại để khôi phục.",
                                        "IN_APP", null);
                }
        }
        ...
        private List<Long> suspendBranchesOverLimit(RestaurantSubscription subscription) {
                List<Branch> activeBranches = branchRepository.findByRestaurantIdAndStatus(
                                subscription.getRestaurant().getId(),
                ...
                long overLimitCount = activeBranches.size() - maxBranchesOnExpiry;
                if (overLimitCount <= 0) {
                        return List.of();
                }
